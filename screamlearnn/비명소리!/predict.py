...
def real_time_predict(model_path):
    ...
    try:
        while True:
            ...
            # 입력 데이터 패딩하여 n_fft 보다 길게 만들기
            if len(audio_data) < 1024:
                padding_needed = 1024 - len(audio_data)
            ...
            # 파일을 이용해 특징 추출
            mfccs, chroma, mel, contrast, tonnetz = extract_feature(temp_filename)
            if None not in [mfccs, chroma, mel, contrast, tonnetz]:
                features = np.hstack([mfccs, chroma, mel, contrast, tonnetz])
                features = np.expand_dims(features, axis=0)
                prediction = model.predict(features)
                label = '비명' if np.argmax(prediction) == 0 else '비비명'
                print(f'예측: {label}')
                if label == '비명':
                    print("비명이 감지되었습니다. 프로그램 종료합니다.")
                    break
            else:
                print("특징 추출에 실패했습니다.")
...
if __name__ == '__main__':
    real_time_predict('trained_model.h5')'''
...
if __name__ == '__main__':
    real_time_predict('trained_model.keras')
